...
namespace ChessBrowser
{
  partial class Form1 {
    ...
    private void insertIntoPlayers(MySqlConnection conn, ChessGame game) {
            // query database for black player and white player, if pID doesn't exist add it,
            MySqlCommand comm = new MySqlCommand("select Elo from Players where Name = @Name", conn);
            comm.Parameters.AddWithValue("@Name", game.WhitePlayer);
            MySqlDataReader reader = comm.ExecuteReader();
            if(!reader.HasRows) {
                reader.Close();
                comm = new MySqlCommand("insert ignore into Players (Name, Elo) values (@Name, @Elo)", conn);
                comm.Parameters.AddWithValue("@Name", game.WhitePlayer);
                comm.Parameters.AddWithValue("@Elo", game.WhiteElo);
                comm.ExecuteNonQuery();
            } else {
                reader.Read();
                if (reader.GetInt32(0) < game.WhiteElo) {
                    reader.Close();
                    comm = new MySqlCommand("update Players p set Elo = @Elo where p.Name = @Name", conn);
                    comm.Parameters.AddWithValue("@Elo", game.WhiteElo);
                    comm.Parameters.AddWithValue("@Name", game.WhitePlayer);
                    comm.ExecuteNonQuery();
            ...
            comm = new MySqlCommand("select Elo from Players where Name = @Name", conn);
            comm.Parameters.AddWithValue("@Name", game.BlackPlayer);
            reader = comm.ExecuteReader();
            if (!reader.HasRows) {
                reader.Close();
                comm = new MySqlCommand("insert ignore into Players (Name, Elo) values (@Name, @Elo)", conn);
                comm.Parameters.AddWithValue("@Name", game.BlackPlayer);
                comm.Parameters.AddWithValue("@Elo", game.BlackElo);
                comm.ExecuteNonQuery();
            }
            else {
                reader.Read();
                int curElo = reader.GetInt32(0);
                if (reader.GetInt32(0) < game.BlackElo) {
                    Console.WriteLine(curElo + "<" + game.BlackElo);
                    reader.Close();
                    comm = new MySqlCommand("update Players p set Elo = @Elo where p.Name = @Name", conn);
                    comm.Parameters.AddWithValue("@Elo", game.BlackElo);
                    comm.Parameters.AddWithValue("@Name", game.BlackPlayer);
            ...
            // if it does and the current elo is bigger, update it
        }
